...
class cOctoBullet;
//class cEnemy;
//extern struct EnemyState;

class cOcto
{
private:
	vector<cOctoBullet * >& m_Bullet;

	Vec2 v_Bornpos;
	
	cTexture * m_Shoot = nullptr;
	...
	float f_BazukaTime;
public:
	cOcto(vector<cOctoBullet *>& m_Bullet,cTexture * MiniMap, EnemyState state, Vec2 Pos, Vec2 PlayerPos, cTexture * PlayerTex, bool ReBorn = false);
	~cOcto();

	void Update(bool follow, Vec2 Target, cTexture * TargetTex);
	void Render();
	void Release();
