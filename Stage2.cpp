...
void Stage2::Init()
{
	...
	mciOpen.lpstrElementName = L"./Sound/Stage2.mp3";
	...
	NowStage = 2;
	m_BackGround = IMAGE->FindImage("Stage2BackGroud");
	m_BackColl = IMAGE->FindImage("Stage2BackColl");
	m_MiniMap = IMAGE->FindImage("Stage2MiniMap");
	m_MiniPlayer = IMAGE->FindImage("MiniPlayer");
	m_ShowMiniMap = IMAGE->FindImage("Stage2ShowMiniMap");
	...
	m_frame->SetFrame(0, 1, 70);

	m_Player->Init(2, m_MiniMap);
	m_Enemy->Init(2, m_MiniMap);
	MiniAlpha = 255;
	...
	BigJFish_Append = IMAGE->FindImage("BigJFish_Append");
	i_Player_x = -1380;
	i_BigJFish_x = 1380;
...
void Stage2::Update()
{
	...
	if (b_Start) {
		...
		if (i_Player_x == 0 && i_BigJFish_x == 0) {
			b_VS = true;
			if (V_Size.x > 1) {
				b_V = true;
				V_Size.x -= 0.03f;
		...
		if (m_Enemy->GetEnemySize() + m_Enemy->GetOctoSize() == 40 && !b_Start && i_Start_y < 820) {
			b_Start = true;
			b_Load = false;
		}
		...
		if (m_Enemy->GetEnemySize() + m_Enemy->GetOctoSize() == 0 && !BossAppend && b_Started) {
			BossAppend = true;
		}

		if (m_Enemy->GetEnemySize() + m_Enemy->GetOctoSize() == 0 && m_Enemy->GetClear())
			b_Clear = true;
	}
}
...
void Stage2::Render()
{
	if (b_Load) {
		RECT GaugeRect = {
			0,
			0,
			((float)m_LoadGauge->info.Width / 40) * (m_Enemy->GetEnemySize() + m_Enemy->GetOctoSize()),
		...
		IMAGE->PrintText("남은 몬스터 : 40 / " + to_string(m_Enemy->GetEnemySize() + m_Enemy->GetOctoSize()), Vec2(1000, 620), 50, D3DCOLOR_XRGB(255, 50, 50), true);
		...
		if (BossAppend) {
			...
			IMAGE->Render(BigJFish_Append, Vec2(i_BigJFish_x, 0));
		}
		if (b_V)
